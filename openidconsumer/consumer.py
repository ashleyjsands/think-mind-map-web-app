...
# This file was modified by Ashley Sands.
#  The root handler class was removed.
#  Imports to simplejson & createUserSession and  was added.
#  Finish handler was modified to set a session cookie and delete the session model.
...
import openidconsumer.fetcher as fetcher
import openidconsumer.store as store

from openiduser.user import createUserSession # Added by Ashley Sands.
...
class LoginHandler(Handler):
  ...
  def post(self):
    ...
    openid_identifier = self.request.get('openid_identifier') # Modified by ashley.
    if not openid_url:
      if openid_identifier:# Modified by ashley.
        logging.info('openid_identifier used.')
        openid_url = openid_identifier# Modified by ashley.
      else:# Modified by ashley.
...
class FinishHandler(Handler):
  ...
  def get(self):
    ...
    self.session.delete()
    createUserSession(self.response, self.session.claimed_id, self.session.server_url,)
